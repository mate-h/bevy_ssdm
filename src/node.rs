...
use bevy::render::{
    ...
    render_graph::{self, NodeRunError, RenderGraphContext, ViewNode},
    render_resource::{
...
#[derive(Debug, Clone, Copy, Hash, PartialEq, Eq, render_graph::RenderLabel)]
pub enum SsdmNode {
    VectorPass,
    Pyramid,
    GBufferWarp,
...
fn run_ssdm_pyramid_passes(world: &World, render_context: &mut RenderContext) {
    ...
    for i in 1..pyramid.a.len() {
        ...
        let du = DownUni {
            in_dims: UVec2::new(in_img.size.width, in_img.size.height),
            out_dims: UVec2::new(out_img.size.width, out_img.size.height),
        };
    ...
    for l in (0..l_count).rev() {
        ...
        let ru = RefineUni {
            dims: UVec2::new(a_img.size.width, a_img.size.height),
            is_coarsest,
            _pad: 0,
        };
        let mut ubuf = UniformBuffer::new(Vec::new());
...
#[derive(Default)]
pub struct SsdmVectorPassNode;

impl ViewNode for SsdmVectorPassNode {
    type ViewQuery = (
...
#[derive(Default)]
pub struct SsdmPyramidPassNode;

impl ViewNode for SsdmPyramidPassNode {
    type ViewQuery = (Option<&'static SsdmSettings>,);

    fn run(
        &self,
        _graph: &mut RenderGraphContext,
        render_context: &mut RenderContext,
        (ssdm_settings,): QueryItem<Self::ViewQuery>,
        world: &World,
    ) -> Result<(), NodeRunError> {
        let Some(settings) = ssdm_settings else {
            return Ok(());
        ...
        run_ssdm_pyramid_passes(world, render_context);
        Ok(())
    }
}

#[derive(Default)]
pub struct SsdmGBufferWarpPassNode;

impl ViewNode for SsdmGBufferWarpPassNode {
    type ViewQuery = (
    ...
    fn run(
        ...
        _graph: &mut RenderGraphContext,
        render_context: &mut RenderContext,
        (prepass, view_depth, ssdm_settings): QueryItem<Self::ViewQuery>,
        world: &World,
    ) -> Result<(), NodeRunError> {
        let Some(settings) = ssdm_settings else {
            return Ok(());
...
#[derive(Default)]
pub struct SsdmPostProcessNode;

impl ViewNode for SsdmPostProcessNode {
    type ViewQuery = (&'static ViewTarget, Option<&'static SsdmSettings>);

    fn run(
        &self,
        _graph: &mut RenderGraphContext,
        render_context: &mut RenderContext,
        (view_target, _ssdm_settings): QueryItem<Self::ViewQuery>,
        world: &World,
    ) -> Result<(), NodeRunError> {
        let pipeline_cache = world.resource::<PipelineCache>();
        let post = world.resource::<SsdmPostPipeline>();
        ...
        // The gather layout reserves bindings for B0 (binding 3) and A0 (binding 4) so the
        // shader can call `resolve_src_uv`, but the post-lighting gather currently runs in
        // passthrough mode (`gather_enabled = 0`). Bind whatever pyramid level 0 textures
        // are available; if the pyramid isn't allocated yet we fall back to `pp.source`
        // for both slots since the layout demands a `texture_2d<f32>` and the shader
        // ignores them in passthrough.
        let pyramid = world.resource::<SsdmPyramidImages>();
        let gpu_images = world.resource::<RenderAssets<GpuImage>>();
        let b0_view = pyramid
            .b
...
pub fn init_ssdm_post_pipeline(
    ...
    let gather_pipeline = pipeline_cache.queue_render_pipeline(RenderPipelineDescriptor {
        ...
        fragment: Some(FragmentState {
            ...
            shader_defs: vec![],
            targets: vec![Some(ColorTargetState {
                format: bevy::render::view::ViewTarget::TEXTURE_FORMAT_HDR,
                blend: None,
                write_mask: ColorWrites::ALL,
            })],
        }),
